...
if (empty($action)) {
	...
	foreach ($mempost as $key => $value) {
		...
		$db->update("UPDATE pw_usergroups SET " . pwSqlSingle(array('grouptitle' => $memtitle[$key],
			'groupimg' => $mempic[$key], 'grouppost' => $mempost[$key])) . " WHERE gptype='member' AND gid=" . pwEscape($key));
	...
	foreach ($deftitle as $key => $value) {
		$db->update("UPDATE pw_usergroups SET " . pwSqlSingle(array('grouptitle' => $value,
			'groupimg' => $defpic[$key])) . " WHERE gptype='default' AND gid=" . pwEscape($key));
	}
	updatecache_l();
	...
	foreach ($systitle as $key => $value) {
		$db->update("UPDATE pw_usergroups SET " . pwSqlSingle(array('grouptitle' => $value,
			'groupimg' => $syspic[$key])) . " WHERE gptype='system' AND gid=" . pwEscape($key));
		updatecache_g($key); /*update cache*/
	}
	updatecache_l();
	...
	foreach ($viptitle as $key => $value) {
		$db->update("UPDATE pw_usergroups SET " . pwSqlSingle(array('grouptitle' => $value,
			'groupimg' => $vippic[$key])) . " WHERE gptype='special' AND gid=" . pwEscape($key));
		updatecache_g($key); /*update cache*/
	}
	updatecache_l();
	...
	if (empty($step)) {
		...
		$grouptitle || $grouptitle = $gp_grouptitle;
		@extract($SYSTEM);
		@extract($_G);
