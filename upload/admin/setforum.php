	...
	//			strlen($descrip[$key])>250 && adminmsg('descrip_long');
		...
		if (empty($_POST['step'])) {
			...
			if ($type <> 'category') {
				...
				//主题分类
				$query = $db->query("SELECT id,name,logo,vieworder,upid,ifsys,isdefault FROM pw_topictype WHERE fid=".S::sqlEscape($fid)." ORDER BY vieworder ");
				$t_typedbnum = 1;
				while ($rt = $db->fetch_array($query)) {
			...
			if ($forum['type'] == 'category') {
				...
				*/
				...
				S::gp(array('creditdb','forumsetdb','uploadset','rewarddb','cfup','ffup','showsub','ifhide', 'viewsub_1','viewsub_2','allowhide','allowrob','allowsell','copyctrl','f_check','password','allowvisit','allowread', 'allowpost','allowrp','allowupload','allowdownload','otherfid','otherforum','allowtime','allowtype', 'recycle','forumsell','sdate','cprice','rprice','logotype','logo_upload','logo_url','ifdellogo','t_view_db','new_t_view_db','t_logo_db','new_t_logo_db','t_sys_db','t_set_default','new_t_sys_db','new_t_sub_logo_db','new_t_sub_view_db','new_t_sub_sys_db','t_type','modelid','pcid','actmid'),'P');
				...
				foreach ($new_t_db as $key => $value) {
					...
					$typedb[] = array ('fid' => $fid,'name' => $value,'logo'=>$new_t_logo_db[$key],'vieworder'=>$new_t_view_db[$key], 'ifsys' => isset($new_t_sys_db[$key]) ? $new_t_sys_db[$key] : 0,'isdefault' => $isdefault);
				}
				if ($typedb) {
					$db->update("REPLACE INTO pw_topictype (fid,name,logo,vieworder,ifsys,isdefault) VALUES " . S::sqlMulti($typedb));
				}
				//增加二级新分类
