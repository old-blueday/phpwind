...
if ($foruminfo['password'] != '' && ($groupid=='guest' || $pwdcheck[$fid] != $foruminfo['password'] && !CkInArray($windid,$manager))) {
...
require_once Pcv(dirname(__FILE__) . "/template/layout/topic.php");

function stopic_load_topic_view($topicType) {
	return Pcv(dirname(__FILE__) . "/template/topic/$topicType.htm");
}
