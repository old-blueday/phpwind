...
if (!$isGM && $winduid != $space['uid']) {
	$userdb = $db->get_one("SELECT index_privacy FROM pw_ouserdata WHERE uid=" . pwEscape($uid));
...
$group = array();
$count = $db->get_value("SELECT COUNT(DISTINCT c.id) AS count FROM pw_cmembers cm LEFT JOIN pw_colonys c ON cm.colonyid=c.id WHERE cm.uid=" . pwEscape($uid) . " AND cm.ifadmin<>'-1'");
...
if ($count) {
	...
	$query = $db->query("SELECT DISTINCT c.* FROM pw_cmembers cm LEFT JOIN pw_colonys c ON cm.colonyid=c.id WHERE cm.uid=" . pwEscape($uid) . " AND cm.ifadmin <> '-1' ORDER BY cm.colonyid DESC $limit");
