...
class GroupStyle {
	...
	*/
	...
	function getGradeStylesByUpid($upids){
		...
		$query = $this->db->query("SELECT * FROM pw_cnstyles WHERE upid IN (".pwImplode($upids).")");
		while ($rt = $this->db->fetch_array($query)) {
	...
	*/
	...
	function addNewSubStyle($newSubStyle) {
		$this->db->update("INSERT INTO pw_cnstyles(cname,ifopen,upid) VALUES ". pwSqlMulti($newSubStyle));
	}
}
?>