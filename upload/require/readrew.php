	...
	if ($lou == 0) {
		if ($rewardtype == '0') {
			...
			if ($groupid=='3' || $admincheck) {
				$html .= "<div class=\"tac\"><a href=\"job.php?action=endreward&tid=$tid\" class=\"s4\">".getLangInfo('bbscode','reward_cancle').'</a>&nbsp;</div>';
			} elseif ($authorid == $winduid && $timeleave < 0) {
				$html .= '<div class="tac"><a href="job.php?action=rewardmsg&tid='.$tid.'" title="'.getLangInfo('bbscode','reward_title').'" onClick="javascript:if(confirm(\''.getLangInfo('bbscode','reward_msgtoadmin').'\')){return true;}else{return false;}"  class=\"s4\">'.getLangInfo('bbscode','reward_toadmin').'</a>&nbsp;</div>';
			}
		} else {
			$html .= "<span class=\"s2\">".getLangInfo('bbscode','reward_finished')."</span><div class=\"tac\">".getLangInfo('bbscode','reward_bestanswer').": $rewdb[cbval]&nbsp;$rw_b_name</div>";
			if ($rewardtype == 1) {
		...
		if ($rewardtype=='1' && $ifreward>1) {
			...
			$html .= "<span class=\"s2\">".getLangInfo('bbscode','reward_manager')."</span>: [<a href=\"job.php?action=reward&tid=$tid&pid=$pid&type=1\" class=\"s4\">".getLangInfo('bbscode','reward_bestanswer')."</a>] ";
