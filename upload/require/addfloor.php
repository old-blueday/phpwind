...
if (getstatus($read['tpcstatus'], 1)) {#帖子是否来自群组
	...
	$colony = $db->get_one("SELECT a.cyid,c.cname,c.ifopen,c.admin,c.speciallevel,c.commonlevel{$_sql_sel} FROM pw_argument a LEFT JOIN pw_colonys c ON a.cyid=c.id{$_sql_tab} WHERE tid=" . pwEscape($tid));
	if ($colony) {
...
if ($read['replies'] > 0 && $topped_page_num < $db_readperpage) {
	if ($openIndex) {#高楼索引处理
		...
		$limit = pwLimit(0,1);
		$query = $db->query("SELECT t.* $fieldadd FROM $pw_posts t $tablaadd WHERE t.tid=".pwEscape($tid)." AND t.pid=".pwEscape($pid)." AND t.ifcheck='1' $sqladd ORDER BY $order $limit");
		while ($read = $db->fetch_array($query)) {
			$_uids[$read['authorid']] = 'UID_'.$read['authorid'];
			$read['aid'] && $_pids[$read['pid']] = $read['pid'];
			$read['istop'] = strpos($read['remindinfo'],getLangInfo('bbscode','read_topped_tag')) !== false ? 'top' : '';
