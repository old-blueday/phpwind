...
class ActivityUpload extends uploadBehavior {
	...
	function update($uploaddb) {
		...
		if ($this->tid) {
			...
			if ($defaultAttach) {
				$this->db->update("UPDATE $defaultValueTableName SET " . pwSqlSingle($defaultAttach)." WHERE tid=". pwEscape($this->tid));
			}
			if ($userAttach) {
				$this->db->update("UPDATE $userDefinedValueTableName SET " . pwSqlSingle($userAttach)." WHERE tid=". pwEscape($this->tid));
			}
		} else {
			foreach ($uploaddb as $key => $value) { 
