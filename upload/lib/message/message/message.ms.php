...
class MS_Message extends MS_Base {
	...
	function _sendSingle($userId,$toUserId,$userName,$categoryId,$typeId,$messageInfo){
		...
		$relations = array(
			array(	'uid'          => $toUserId,
					...
					'typeid'       => $typeId,
					'status'       => $this->_s_not_read,
					'isown'        => $this->_s_other,
					'created_time' => $this->_timestamp,
					'modified_time'=> $this->_timestamp,
			),
			array(	'uid'          => $userId,
					'mid'          => $messageId,
					'categoryid'   => $categoryId,
					'typeid'       => $typeId,
					'status'       => $this->_s_have_read,
					'isown'        => $this->_s_self,
					'created_time' => $this->_timestamp,
					'modified_time'=> $this->_timestamp,
			),
	...
	function getUpMessage($userId,$relationId,$typeId){
		...
		return $this->_upMessage($userId,$this->_sms,$typeId,$relationId);
	}
	function getDownMessage($userId,$relationId,$typeId){
		...
		return $this->_downMessage($userId,$this->_sms,$typeId,$relationId);
	}
	...
	function checkReceiver($usernames){
		...
		list($userId,$username,$groupId) = array($user['uid'],$user['username'],(($user['groupid'] > 0) ? $user['groupid'] : $user['memberid']) );
