...
class MS_Base {
	...
	function _upMessage($userId,$category,$typeId,$relationId){
		$userId = intval($userId);
		$categoryId = intval($this->getMap($category));
		$typeId = intval($typeId);
		$relationId = intval($relationId);
		if( 1 > $userId || 1 > $categoryId || 1 > $typeId || 1 > $relationId ){
			return false;
		}
		$relationsDao = $this->getRelationsDao();
		...
		if(!$relation = $relationsDao->getUpRelation($userId,$categoryId,$typeId,$relationId,$tmpRelation['modified_time'])){
			return false;
		}
	...
	function _downMessage($userId,$category,$typeId,$relationId){
		$userId = intval($userId);
		$categoryId = intval($this->getMap($category));
		$typeId = intval($typeId);
		$relationId = intval($relationId);
		...
		if(!$relation = $relationsDao->getDownRelation($userId,$categoryId,$typeId,$relationId,$tmpRelation['modified_time'])){
			return false;
		}
		if(!$message = $this->_get($relation['mid'])){
			return false;
	...
	function _reverseString($content){
		return str_replace(array('"'.$this->_userName.'"','['.$this->_userName.']','&quot;'.$this->_userName.'&quot;'),'您',$content);
	}
	...
	function _addSearch($userId,$toUserId,$relationId,$messageId,$typeId){
		$fieldData = array(	'rid'          => $relationId,
							'uid'          => $toUserId,
							'mid'          => $messageId,
							'typeid'       => $typeId,
							'create_uid'   => $userId,
	...
	function maps2(){
		return array(
			$this->_sms        => array($this->_sms_message,$this->_sms_rate,$this->_sms_comment,$this->_sms_guestbook,$this->_sms_share,$this->_sms_reply),
			$this->_notice     => array($this->_notice_system,$this->_notice_postcate,$this->_notice_active,$this->_notice_apps),
			$this->_request    => array($this->_request_friend,$this->_request_group,$this->_request_active,$this->_request_apps),
	...
	function getStatisticsByCategory($category){
		$maps = array(
			$this->_sms      => $this->_c_sms_num, 
			$this->_notice   => $this->_c_notice_num,
			$this->_request  => $this->_c_request_num,
			$this->_groupsms => $this->_c_groupsms_num,
		);
