...
class PW_Ms_RelationsDB extends BaseDB {
	...
	function addRelations($fieldDatas){
		$this->_db->update("INSERT INTO ".$this->_tableName." (uid,mid,categoryid,typeid,status,isown,created_time,modified_time) VALUES  ".pwSqlMulti($fieldDatas,FALSE));
	...
	function getUpRelation($userId,$categoryId,$typeId,$relationId,$modified_time){
		return  $this->_db->get_one ( "SELECT * FROM " . $this->_tableName. " WHERE uid = ".$this->_addSlashes($userId)." AND categoryid=".$this->_addSlashes($categoryId)." AND typeid=".$this->_addSlashes($typeId)." AND modified_time > ".$this->_addSlashes($modified_time)." ORDER BY modified_time ASC LIMIT 1" );
	}
	function getDownRelation($userId,$categoryId,$typeId,$relationId,$modified_time){
		return  $this->_db->get_one ( "SELECT * FROM " . $this->_tableName. " WHERE uid = ".$this->_addSlashes($userId)." AND categoryid=".$this->_addSlashes($categoryId)." AND typeid=".$this->_addSlashes($typeId)." AND modified_time < ".$this->_addSlashes($modified_time)." ORDER BY modified_time DESC LIMIT 1" );
	}
	...
	function deleteRelationsByUserIdAndCategoryId($userId,$categoryIds){
		$categoryIds = is_array($categoryIds) ? pwImplode($categoryIds) : $categoryIds;
	...
	function countAllByUserIds($userIds,$typeIds){
		$query = $this->_db->query (  "SELECT COUNT(*) as total,uid FROM " . $this->_tableName. " WHERE uid in( ".pwImplode($userIds).") AND typeid in( ".pwImplode($typeIds).") GROUP BY uid ORDER BY null" );
		return $this->_getAllResultFromQuery ( $query );
	}
