...
class PW_PageInvokeDB extends BaseDB {
	...
	function searchPageInvokes($array,$page,$prePage = 20) {
		...
		$temp = array();
		$_sql = "SELECT p.*,i.title FROM ".$this->_tableName." p LEFT JOIN ".$this->_joinTable." i ON p.invokename=i.name".$this->_getSearchSql($array,1).' ORDER by p.state ASC, p.id ASC '.pwLimit($page*$prePage,$prePage);
	...
	function _getSearchSql($array,$ifJoin=0) {
		...
		foreach ($array as $key=>$value) {
			if ($key == 'invokename') {
				$_sql_where .= " AND ".($ifJoin ? "p.":'')."invokename LIKE ".pwEscape("%$value%");
			} elseif ($key == 'sign' && $value) {
				$_sql_where .= " AND ".($ifJoin ? "p.":'')."sign=".pwEscape($value);
			} else {
				$_sql_where .= " AND ".($ifJoin ? "p.":'')."$key=".pwEscape($value);
			}
		}
		return $_sql_where;
	}
	...
	function deleteByScrAndSign($scr,$sign) {
		$this->_db->update("DELETE FROM ".$this->_tableName." WHERE scr=".pwEscape($scr)." AND sign=".pwEscape($sign));
	}
	function getEffectPageInvokes($scr,$sign,$ifverify = 0) {
		$temp = array();

		$ifverify = (int) $ifverify;
		$_sql_add = $ifverify ? " AND p.ifverify=".pwEscape($ifverify): '';
		$query = $this->_db->query("SELECT p.*,i.title FROM ".$this->_tableName." p LEFT JOIN ".$this->_joinTable." i ON p.invokename=i.name WHERE p.scr=".pwEscape($scr)." AND p.sign=".pwEscape($sign)." AND p.state=0 $_sql_add");
		while ($rt = $this->_db->fetch_array($query)) {
			$rt = $this->_unserializeData($rt);
			$temp[] = $rt;
		}
	...
	function getPageInvokes($scr,$sign) {
		$temp = array();
		$query = $this->_db->query("SELECT * FROM ".$this->_tableName." WHERE scr=".pwEscape($scr)." AND sign=".pwEscape($sign));
		while ($rt = $this->_db->fetch_array($query)) {
			$rt = $this->_unserializeData($rt);
			$temp[] = $rt;
		}
	...
	function updatePageInvokesState($scr,$sign,$invokeNames,$state) {
		if ($invokeNames && is_array($invokeNames)) {
			$this->_db->update("UPDATE ".$this->_tableName." SET state=".pwEscape($state)." WHERE scr=".pwEscape($scr)." AND sign=".pwEscape($sign)." AND invokename IN(".pwImplode($invokeNames).')');
		} elseif ($invokeNames) {
			$this->_db->update("UPDATE ".$this->_tableName." SET state=".pwEscape($state)." WHERE scr=".pwEscape($scr)." AND sign=".pwEscape($sign)." AND invokename=".pwEscape($invokeNames));
		} else {
			$this->_db->update("UPDATE ".$this->_tableName." SET state=".pwEscape($state)." WHERE scr=".pwEscape($scr)." AND sign=".pwEscape($sign));
		}
	}
	//在模块被重复使用时会有问题
	function getSignByInvokeName($name) {
		return $this->_db->get_value("SELECT sign FROM ".$this->_tableName." WHERE invokename=".pwEscape($name));
	}
	
	function getByUnique($scr,$sign,$invokeName){
		$temp = $this->_db->get_one("SELECT * FROM ".$this->_tableName." WHERE scr=".pwEscape($scr)." AND sign=".pwEscape($sign)." AND invokename=".pwEscape($invokeName));
		return $this->_unserializeData($temp);
	}
