...
class PW_PageCacheDB extends BaseDB {
	...
	function get($sign) {
		$temp = $this->_db->get_one('SELECT * FROM '.$this->_tableName.' WHERE sign='.pwEscape($sign) );
	...
	function gets($array) {
		...
		$rs = $this->_db->query('SELECT * FROM '.$this->_tableName.' WHERE sign IN ('.pwImplode($array).')');
	...
	function updates($array){
		...
		$this->_db->update("REPLACE INTO ".$this->_tableName." (sign,data,cachetime) VALUES " . pwSqlMulti($array,false));
	}
	function update($sign,$array){
		$array	= $this->_checkData($array);
		if (!$array) return null;
		$this->_db->update('UPDATE '.$this->_tableName.' SET '.pwSqlSingle($array,false).' WHERE sign='.pwEscape($sign));
	}
	function delete($sign) {
		$this->_db->update('DELETE FROM '.$this->_tableName.' WHERE sign='.pwEscape($sign));
	}
	function truncate() {
		$this->_db->update('TRUNCATE TABLE '.$this->_tableName);
	}
	function deleteByType($type) {
		$this->_db->update('DELETE FROM '.$this->_tableName.' WHERE type='.pwEscape($type));
	}
