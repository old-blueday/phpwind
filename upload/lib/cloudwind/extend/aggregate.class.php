...
/**
 * phpwind版本通用查询语句分析器（基础正则版）
 * for 帖子/回复/用户/版块/日志/群组 操作日志功能
 * 
 * @author Liu Hui <developer.liuhui@gmail.com> 2011-2-21
...
class PW_Aggregate {
	...
	function PW_Aggregate() {
		$this->__construct ();
	}
	
	function __construct() {
		if (! isset ( $this->_service ['parse'] ) || ! $this->_service ['parse']) {
			$this->_service ['parse'] = new Aggregate_SQLParseExtension ();
		}
		if (! isset ( $this->_service ['operate'] ) || ! $this->_service ['operate']) {
			$this->_service ['operate'] = ($GLOBALS ['db_yun_model'] ['search_model'] == 100) ? new Aggregate_OperateLog () : new Aggregate_SyncOperateLog ();
		}
		if ($this->_doubleModel && (! isset ( $this->_service ['operate_normal'] ) || ! isset ( $this->_service ['operate_sync'] ) || ! $this->_service ['operate_normal'] || ! $this->_service ['operate_sync'])) {
			$this->_service ['operate_normal'] = new Aggregate_OperateLog ();
			$this->_service ['operate_sync'] = new Aggregate_SyncOperateLog ();
		}
	}
	
	function collectSQL($sql, $db) {
		list ( $operate, $tableName, $fields ) = $this->_service ['parse']->parseSQL ( $sql, $db );
		if (! $operate) {
			return false;
		}
...
class Aggregate_SyncOperateLog {
